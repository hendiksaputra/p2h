...
export function VehiclesToolbar({ initialQ }: Props) {
  ...
  useEffect(() => {
    const handle = window.setTimeout(() => {
      ...
      startTransition(() => {
        router.replace(nextQs ? `/vehicles?${nextQs}` : "/vehicles");
      });
    }, 320);
    ...
    <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
      <div className="relative w-full sm:max-w-md">
        ...
        <input
          ...
          type="search"
          autoComplete="off"
          placeholder="Cari nomor polisi, unit no, merek, model…"
          value={value}
      ...
      <p className="text-xs text-slate-500">Pencarian memperbarui daftar secara otomatis.</p>
    </div>
  );
}
